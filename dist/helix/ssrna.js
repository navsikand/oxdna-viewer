...
(function (honda) {
    ...
    honda.findrnapartials = findrnapartials;
    ...
    honda.rnaseparation = rnaseparation;
    function rnaGenerateHelix(partials, deadfishies = []) {
        ...
        const getNodeRef = (nt) => {
            ...
            if (dIdx !== undefined)
                return { kind: 'deadfishy', index: dIdx };
            return null;
        };
        ...
        const addDeadfishyLink = (deadfishyIdx, partialIdx) => {
            const set = deadfishyLinks.get(deadfishyIdx) || new Set();
            set.add(partialIdx);
    ...
    honda.rnaGenerateHelix = rnaGenerateHelix;
})(honda || (honda = {}));
